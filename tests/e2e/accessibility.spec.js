...
test("każda główna trasa przechodzi automatyczny audyt WCAG w jasnym motywie", async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem("sagSetupLogbookWeb.settings.v1", JSON.stringify({ appearanceMode: "light" })));
  await gotoApp(page);
  await auditRoutes(page);
});

test("każda główna trasa przechodzi automatyczny audyt WCAG w ciemnym motywie", async ({ page }) => {
  await page.addInitScript(() => localStorage.setItem("sagSetupLogbookWeb.settings.v1", JSON.stringify({ appearanceMode: "dark" })));
  await gotoApp(page);
