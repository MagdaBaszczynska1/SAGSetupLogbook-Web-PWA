...
export function createResetDialog() {
  const element = createElement("dialog", {
    ...
    attributes: { "data-dialog": "reset", "aria-labelledby": "measurement-reset-title" }
  });
  element.innerHTML = `
    <form method="dialog" class="dialog-panel">
      <div class="dialog-heading"><h3 id="measurement-reset-title">Wyczyścić formularz?</h3><button value="cancel" aria-label="Zamknij">×</button></div>
...
export function createSavedDialog() {
  const element = createElement("dialog", {
    className: "app-dialog",
    attributes: { "data-dialog": "saved", "aria-labelledby": "measurement-saved-title" }
  });
  element.innerHTML = `
    <div class="dialog-panel">
      <div class="dialog-heading"><h3 id="measurement-saved-title">Pomiar zapisany</h3><button type="button" data-action="close-saved" aria-label="Zamknij">×</button></div>
