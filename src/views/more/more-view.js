...
export function createMoreView({ bikeStore, measurementStore, rideJournalStore, database, appSettingsStore, persistence, pwaManager }) {
  const screen = createElement("section", { className: "screen more-screen", attributes: { "aria-labelledby": "more-heading" } });
  ...
  const storageStatus = createElement("div", {
    ...
    children: [
      ...
      createElement("span", { text: persistence.isPersistent ? "Profile, pomiary i wpisy Dziennika korzystają z lokalnej bazy IndexedDB." : "Dane znikną po zamknięciu karty. Od razu utwórz kopię JSON, jeżeli chcesz je zachować." })
    ]
  });
  storageStatus.dataset.tone = persistence.isPersistent ? "success" : "warning";
  const tools = createElement("section", { className: "more-tools", attributes: { "aria-labelledby": "more-tools-heading" } });
  tools.append(
    createElement("h2", { text: "Pomoc i ustawienia", attributes: { id: "more-tools-heading" } }),
    createElement("div", { className: "more-action-grid", children: [
  ...
  const dataManagement = createDataManagementView({ database, bikeStore, measurementStore, rideJournalStore, appSettingsStore });
  const about = createElement("section", {
    className: "about-card app-card",
    attributes: { "aria-labelledby": "about-heading" },
    children: [
      createElement("div", { className: "more-section-heading", children: [createElement("div", { children: [
        createElement("h2", { text: "O aplikacji", attributes: { id: "about-heading" } }),
        createElement("p", { text: `SAG Setup Logbook — wersja webowa ${APP_VERSION}` })
      ] })] }),
  ...
  screen.append(intro, storageStatus, createBikesView({ bikeStore }), pwaInstall, tools, dataManagement, about, guideDialog, settingsDialog, privacyDialog);
  screen.addEventListener("click", event => {
    const button = event.target.closest("button[data-action], button[data-appearance-mode]");
    if (!button) return;
    if (button.dataset.appearanceMode) { appSettingsStore.setAppearanceMode(button.dataset.appearanceMode); return; }
